...
class MatchingThread(QtCore.QThread):
    ...
    def end_do(info):
        ...
        :return: 无
        """
        # Accept either a UIInfo instance or an indexable sequence
        if isinstance(info, UIInfo):
            i = info
        else:
            i = UIInfo.from_sequence(info)

        if_end = i.if_end
        handle_title = i.handle_title
        handle_num_list = i.handle_num
        process_num = i.process_num
        connect_mod = i.connect_mod
    ...
    def get_ui_info(self):
        ...
        # 仅支持 Windows 程序窗体 模式
        connect_mod = self.ui_info.rd_btn_windows_mod.text() if self.ui_info.rd_btn_windows_mod.isChecked() else None
        ...
        # 界面设置的参数写入配置文件 (writ_config_ui_info accepts UIInfo or list)
    ...
    def run(self):
        ...
        # support both UIInfo and tuple/list shapes
        if isinstance(info, UIInfo):
            cfg = info
        else:
            cfg = UIInfo.from_sequence(info)

        if cfg.connect_mod == "Windows程序窗体" and cfg.process_num == "多开" and cfg.handle_num == '0':  # 检测如果选择多开，是否已经获取句柄编号
            # 多开使用循环，每个循环针对一个窗口
            print("<br>请点击【选择窗体】获取目标窗体的句柄编号，支持选择多个游戏窗口！")
            self.finished_signal.emit(True)
            return

        run_times_mode = cfg.times_mode
        debug_status = cfg.debug_status
        set_priority_status = cfg.set_priority_status
        interval_seconds = [cfg.interval_seconds, cfg.interval_seconds_max]
        start_match = StartMatch(cfg.to_list()[:12])
        loop_seconds = int(cfg.loop_min * 60)
        ...
        if run_times_mode == "按分钟计算":
            ...
            end_time = 9999999999
            run_rounds = int(cfg.loop_min)

        # 生成随机点击模型，其中info[3]是随机偏移像素值，这里作为点击模型的坐标范围，
        # 如：偏移50，则模型的坐标范围为[(-50,50),(-50,50)]的正态分布数组
        click_mod1 = ClickModSet.create_click_mod(cfg.click_deviation)  # 精确模型，用于关键图片偏移，偏移量可设置
        ...
        # StartMatch expects a list-like summary of initial params — pass the first 12 entries
        init_value = start_match.set_init(cfg.to_list()[:12])
